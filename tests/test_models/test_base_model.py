...
class TestBaseModel(unittest.TestCase):
    ...
    def test_datetime_attributes(self):
        """Test datetime attributes of BaseModel instances."""
        tic = datetime.utcnow()
        inst1 = BaseModel()
        toc = datetime.utcnow()
        self.assertTrue(
            tic <= inst1.created_at <= toc,
            f"Created_at time {inst1.created_at} "
            f"is not within range {tic} to {toc}"
        )

        time.sleep(1)  # Sleep to ensure a difference
        tic = datetime.utcnow()
        inst2 = BaseModel()
        toc = datetime.utcnow()
        self.assertTrue(
            tic <= inst2.created_at <= toc,
            f"Created_at time {inst2.created_at} "
            f"is not within range {tic} to {toc}"
        )
